...
export function DomainsHero() {
  ...
  const [handles, setHandles] = useState(DEMO_HANDLES);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const extensions = TLDS.map((t) => t.ext);
      const results = await checkDomains(slug, extensions);

      // Map the results back to the TLDS structure
      const mappedDomains = TLDS.map(({ ext, price }) => {
        ...
        return {
          name: fullDomainName,
          available: apiResult ? apiResult.available : false,
          price,
        };
      });

      setDomains(mappedDomains);
      // Update social handles with simulated logic as requested
      setHandles(generateHandles(slug));
    } catch (err: any) {
      console.error(err);
      setError(err.message || "An error occurred while checking domains.");
      // Fallback to offline simulation if the API call fails
      setDomains(generateDomains(slug));
      setHandles(generateHandles(slug));
    } finally {
      setIsLoading(false);
    }
        ...
        {showCard && (
          ...
          >
            ...
            <form onSubmit={handleSearch} className="mb-4">
              <div className="flex h-16 items-center justify-between gap-2 rounded-full border border-[#dcd2c5] bg-[rgba(244,238,228,0.3)] pl-4 pr-2.5 py-2">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="inline-flex h-10 items-center justify-center rounded-full bg-coral-500 px-5 font-figtree text-base font-normal text-white transition-colors hover:bg-coral-600 active:scale-95 disabled:bg-coral-400 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
            ...
            <div className="flex flex-col gap-4 relative">
              {/* Loading overlay */}
              {isLoading && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/70 backdrop-blur-[2px] rounded-2xl transition-all duration-300">
                  <div className="flex flex-col items-center gap-3 bg-white border border-[#f4eee4] shadow-lg rounded-2xl p-6">
